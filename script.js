document.addEventListener('DOMContentLoaded', function() {
    ...
    function renderTask(task) {
        ...
        taskElement.className = 'task-card bg-white p-3 rounded-md shadow cursor-move border-l-4';
        ...
        taskElement.innerHTML = `
            ...
            <p class="text-sm text-gray-600 mt-1 line-clamp-2">${task.description || 'No description'}</p>
            <div class="flex justify-between items-center mt-3">
                <span class="text-xs text-gray-500">${formatDate(task.dueDate)}</span>
                <span class="priority-${task.priority} px-2 py-1 text-xs font-semibold rounded-full">${task.priority}</span>
            </div>
        `;
    ...
    function showTaskDetails(taskId) {
        ...
        document.getElementById('detail-created-at').textContent = `Created on ${formatDate(task.createdAt)}`;
